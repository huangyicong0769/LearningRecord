...
inline void solve() {
  ...
  for (int i = 1; i <= min((ll)(n + 1), k); i++)
    f[i] = f[i - 1] * inv1 % mod * po(i, mod - 2) % mod * (k - i + 1) %
           mod;  //递推f数组
  for (int i = 1; i < n; i++)
    for (int j = 1; j <= i; j++)
      mp[i][j] = (f[i - j] * m + f[i - j + 1]) % mod * inv % mod;  //列方程
  for (int i = 1; i < n; i++) mp[i][i + 1] = f[0] * inv % mod;     // i+1
  for (int i = 1; i <= n; i++) mp[i][n + 1] = mod - 1;             //常数项
  for (int i = 1; i <= n; i++) mp[n][i] = f[n - i];
  ...
  for (int i = 1; i <= n; i++)  //高斯消元
  {
    ...
    for (int j = i + 1; j <= n; j++) {
      ll mult = mp[j][i];
      mp[j][i] = 0;
      (mp[j][i + 1] += mod - mult * mp[i][i + 1] % mod) %= mod;
