...
const int Len = 2333333,
          aa[18] = {1,   2,    4,    8,    16,   32,    64,    128,   256,
...
vd Dfs(int u) {
  dep[u] = dep[fa[0][u]] + 1, dp[1][u] = p[u],
  f[0][0][0][u] = INF;  //相邻点不能都为0
  for (rg int i = 1; aa[i] < dep[u]; ++i) fa[i][u] = fa[i - 1][fa[i - 1][u]];
  for (rg int i = h[u]; i; i = e[i].nxt) {
    int v = e[i].to;
    if (v != fa[0][u])
      fa[0][v] = u, Dfs(v), dp[0][u] += dp[1][v],
...
vd Cfs(int u) {
  ...
  for (rg int i = 1; aa[i] < dep[u]; ++i) {
    int F = fa[i - 1][u];
    f[0][0][i][u] = Min(f[0][0][i - 1][u] + f[0][0][i - 1][F],
                        f[0][1][i - 1][u] + f[1][0][i - 1][F]),
    f[0][1][i][u] = Min(f[0][0][i - 1][u] + f[0][1][i - 1][F],
                        f[0][1][i - 1][u] + f[1][1][i - 1][F]),
    f[1][0][i][u] = Min(f[1][0][i - 1][u] + f[0][0][i - 1][F],
                        f[1][1][i - 1][u] + f[1][0][i - 1][F]),
    f[1][1][i][u] = Min(f[1][0][i - 1][u] + f[0][1][i - 1][F],
                        f[1][1][i - 1][u] + f[1][1][i - 1][F]);
  }  // 4种情况的合并
  for (rg int i = h[u]; i; i = e[i].nxt)
    if (e[i].to != fa[0][u]) Cfs(e[i].to);
}
il vd Work() {
  ...
  for (rg int i = Log[dep[u] - dep[v]]; i >= 0; --i)
    if (dep[u] - aa[i] >= dep[v]) {
      ll t0 = u0, t1 = u1;
  ...
  if (u == v)
    L = u, y ? l1 = u1 : l0 = u0;  //在1条链上
  else {
    for (rg int i = Log[dep[u] - 1]; i >= 0; --i)
      if (fa[i][u] != fa[i][v]) {
    ...
    l1 = dp[1][L] - Min(dp[0][u], dp[1][u]) - Min(dp[0][v], dp[1][v]) +
         Min(u0, u1) + Min(v0, v1);
  }  // 注意这里减去两个儿子的贡献
  // printf("%d\n",L);
  if (L == 1)
    ...
    for (rg int i = Log[dep[L] - 2]; i >= 0; --i)
      if (dep[L] - aa[i] > 1) {
        ll t0 = l0, t1 = l1;
    ...
    ans = Min(dp[0][1] - dp[1][L] + l1,
              dp[1][1] - Min(dp[0][L], dp[1][L]) + Min(l0, l1));
  }
  rt(ans < INF ? ans : -1), pc('\n');
}
int main() {
