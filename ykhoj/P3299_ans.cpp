...
inline void _debug(const ll a[70][70]){
    for (register int i=0; i<=m*n; printf("\n"), i++) for (register int j=0; j<=m*n; j++) printf("%lld ", a[i][j]);
    printf("\n");
}
inline void _debug(const ll a[70]){
    for (register int i=0; i<=m*n; i++) printf("%lld ", a[i]);
    printf("\n\n");
}
