...
void dfs2(LL x, bool n = false) {
  ...
  if (siz[x] > 1) {
    LL l = 0;
    for (LL i = pre[x]; ~i; i = nxt[i])
      if (to[i] != fa[x] && siz[to[i]] > siz[l]) l = to[i];
    dfs2(l);
    for (LL i = pre[x]; ~i; i = nxt[i])
      if (to[i] != fa[x] && to[i] != l) dfs2(to[i], true);
  }
}

inline void work() {
  memset(pre, -1, sizeof pre);
  for (LL i = 1; i < n; ++i) addEdge(readInt(), readInt());
...
inline void maintain(LL o, LL l, LL r) {
  ...
  if (l == r) {
    ...
    ss2[o] = ss2[lch] + ss2[rch] + addv[o] * addv[o] * (r - l + 1) +
             2 * addv[o] * (ss[lch] + ss[rch]);
  }
}
...
int main() {
  ...
  TreeChain::work();
  sa = TreeChain::s[1];
  for (int i = 1; i <= n; i++) printf("%d ", TreeChain::s[i]);
  puts("");
  SegTree::build(1, 1, n);
  while (q--) {
    if (readInt() == 1) {
